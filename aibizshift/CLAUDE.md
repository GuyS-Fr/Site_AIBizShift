...
- **Build Coolify echoue `ERR_PNPM_UNSUPPORTED_ENGINE` (Got: 11.9.0, Expected: ^9 || ^10) (2026-07-01):** Le Dockerfile fait `corepack enable pnpm` sans version epinglee → corepack telechargeait **pnpm 11 (latest)**, rejete par `engines.pnpm` (`^9 || ^10`). En prime, pnpm 11 ignore le champ `pnpm` de package.json (`pnpm.overrides`, `pnpm.onlyBuiltDependencies` → l'override de securite drizzle-orm aurait saute). Fix : ajout du champ `"packageManager": "pnpm@10.24.0"` dans `package.json` — corepack lit ce champ et utilise exactement cette version dans les deux stages Docker (`deps` + `builder`). ⚠️ Garder ce pin aligne avec la version pnpm locale qui genere le `pnpm-lock.yaml` (lockfileVersion 9.0, compatible pnpm 9/10).
