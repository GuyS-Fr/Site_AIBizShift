# PROMPT CLAUDE CODE — Homepage AIBizShift (avec images)
...
Fond : image `hero-background.png` en background cover, avec un overlay sombre semi-transparent pour garantir la lisibilité du texte.
...
- CTA principal : "Réserver mon audit gratuit" → href="#audit"
...
Image `hero-illustration.png` (l'image réseau neuronal + icônes dorées d'entreprises).
...
Image en haut de la card : hauteur fixe h-48, object-cover, w-full. Léger zoom au hover du groupe (group-hover:scale-105 transition-transform duration-500).

Contenu sous l'image : p-8

Icône SVG (w-10 h-10, couleur #3B82F6) au-dessus du titre. Utilise des icônes SVG inline simples :
- Card 1 : engrenage/cog (consulting)
- Card 2 : globe (sites web)
- Card 3 : academic cap (formation)
- Card 4 : rocket (SaaS)
...
"Voir tous nos services →" (text-blue-500 hover:text-blue-700 font-semibold text-lg transition-colors)
...
Style : rounded-xl shadow-2xl w-full max-w-md mx-auto, légère rotation (rotate-2 ou -rotate-2) pour un effet dynamique.
...
- Liens (text-slate-400 hover:text-white transition-colors, espacement mt-2 entre chaque) :
  Services, Formations, Portfolio, Blog, À propos

**Colonne 3 (services) :**
- Titre : "Services" (même style)
- Liens : Consulting IA, Sites web, Formation, SaaS sur mesure, Conciergerie IA

**Colonne 4 (contact) :**
- Titre : "Contact" (même style)
- "contact@aibizshift.eu" (text-slate-400 hover:text-white)
- Lien "LinkedIn →" (text-slate-400 hover:text-white mt-2)
- Lien "Malt →" (text-slate-400 hover:text-white mt-2)
- Bouton petit : "Audit gratuit →" (mt-4 inline-block text-sm bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition)
...
3. Pour l'image hero en fond, utilise soit un `<div>` avec `style={{ backgroundImage }}` soit le composant Image avec `fill` et `className="object-cover"` dans un container relatif.
4. Assure-toi que tous les liens internes (#audit, #services) fonctionnent en smooth scroll. Ajoute `scroll-smooth` sur le html ou le container parent.
5. La page doit être 100% responsive : teste mentalement chaque section en mobile (375px), tablette (768px), desktop (1280px).
6. Le composant doit être un Server Component (pas de "use client") sauf si tu as besoin d'interactivité (dans ce cas, isole les parties client dans des sous-composants).
7. Vérifie le routing existant du template Payload avant de modifier — ne casse pas les routes `/admin`, `/api`, etc.
8. Ajoute la classe `scroll-smooth` quelque part pour le smooth scrolling des ancres.
...
Les images suivantes doivent être référencées dans le code :

1. `hero-background.png` (1920x1080) — fond du hero, utilisé en background
2. `hero-illustration.png` (800x800) — illustration réseau neuronal + entreprises, colonne droite du hero
...
Si une image n'existe pas encore, utilise un placeholder avec un div coloré en bg-slate-700 avec la mention "Image à venir" en text-slate-400 centré — cela permettra de développer la page maintenant et d'ajouter les images après.
